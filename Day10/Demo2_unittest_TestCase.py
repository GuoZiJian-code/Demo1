...
import unittest
from Day10.Math_Methods import Math_Method
...
class TestMathMethod_add(unittest.TestCase):
    ...
    def test_add_two_positive(self):
        ...
        self.assertEqual(2,result,msg="预期结果：{0}，结果：{1}".format(2,result))

    # 2.两个负数相加
    def test_add_two_nagetive(self):
        result = Math_Method(-1, -2).add()
        print("-1和-2相加，结果：{0}，预期结果：{1}".format(result, -3))
        self.assertEqual(-3, result,msg="预期结果：{0}，结果：{1}".format(-3,result))
...
class TestMathMethod_multi(unittest.TestCase):
    def test_multi_two_positive(self):
        ...
        self.assertEqual(2, result,msg="预期结果：{0}，结果：{1}".format(4,result))
